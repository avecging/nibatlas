...
| `report-listing` | The correction form for TY Lee Pen Shop. The listing is the page's heading, not a field: the reader arrived from it, so nothing here asks which shop they mean. The category list leads with the two entries that are not faults — something missing, and a shop doing more than we listed |
...
| `submission-sent` | The confirmation, which replaces the form only after the route answered `ok`, and offers a blank form for a second suggestion. *Thanks for contributing!*, in our own voice rather than a support-ticket register |
