...
**Regenerated 30 August 2026** after the founder's second staging review: the
Save bookmark leads a fixed two-column title row so a long or wrapped name cannot
move it, operational status has three levels of attention (green `Open`, filled
amber for a confirmed closure, a soft amber outline for `Status not confirmed`),
and the preflight's confirm button is Atlas Navy.

Regenerated 28 August 2026 after the first staging review: the actions are
...
| `shop-gap` | SKB, the thinnest record: the restrained `Status not confirmed` outline, one caution for the unknown value layer, one for the unpublished hours, no invented address, and no `Getting there` heading over an empty subsection |
...
| `header-actions` | The revised header on Aesthetic Bay, above the fold: the Save bookmark leading the title row, the green `Open` badge, then Directions and the Plum `Collect Stamp` |
| `preflight` | The collection preflight on Aesthetic Bay: `I am at this shop` in Atlas Navy — a confirmation, not the collectible action — with Cancel quiet beneath it |
| `long-name-title` | Ginza Itoya Yokohama Motomachi at each width: the bookmark holds its column while the Japanese name wraps beside it |
