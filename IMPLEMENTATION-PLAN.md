...
| M6 WP1/WP2 | Authorization/audit and shop operations implemented. Founder confirmed draft creation; broader admin acceptance is separate. The seven-section editor rebuild answering the 20 September founder feedback is on `claude/nib-atlas-admin-rebuild-20rvho`; its backend dependencies merged through #80/#79 at `dc063747` and deployed successfully (35498543648). Founder reports checks 1–6 otherwise work, except logo intake; `fix/logo-image-intake` addresses that rejection and requested automatic resizing, pending review/CI/deployment/retest. |
...
D1 carries existing B/C data without schema/API changes: weekly Unknown/Open/Closed
entries, split/overnight spans, full-day copying, deliberate clearing and inline
validation; stable caption edits across media refresh/conflict; separate private
save, gallery-save and publication wording; read-only latest-saved comparison
following a failed save; mobile/desktop saved-text preview with explicit exclusions.
The existing private/public revision, location invalidation and C3 re-review rules
are unchanged. Gallery arrangement still affects approved images immediately;
private images remain private. No new combined publication transaction is implied.

Remaining **within D**, not reassigned to E/F: structured dated exceptions (requires
an additive normalizer/SQL/public-decoder contract across manual/import consumers),
actual-public-renderer preview parity, pending photo/logo/stamp selection integrated
with revision-bound review and recoverable partial media outcomes. Existing prose
holiday notes and separate media/artwork review remain usable. D1 does not claim
the complete approved editor or founder acceptance. Public-page/map redesign #87,
catalogue research, hosted data/migrations and production are excluded.

D2 merged in PR #96 at `515034bfc7b02e337c01da14ded8e1c5f0cbfccf`;
the founder reports it generally works. It adds structured dated exceptions and
corrects selected-import-filename feedback. D3 is the bounded saved-preview slice:
the actual public renderer in real 360/1280 px viewports, exact saved revision,
approved photos/logo via private authorized delivery and truthful preview-only
review wording. The actions are inert and no nearby/account state is simulated.
Incomplete drafts keep editing/Fix available and receive a clear incomplete-preview
notice, rather than invented position/type data. A small read-only options migration
aligns tied vocabulary ordering with the public projection. Full integrated pending
photo/logo/stamp decisions, revision-bound combined review and recoverable partial
publication outcomes remain **within D**, next after D3. The preview is not that
publication transaction and approved-media reads are separate current snapshots.
Map tile report #97 is excluded from current Package D work; the founder reports it is no longer an issue. No hosted changes
or deployment are authorized by these checkpoints.

### Package D4b — integrated selected preview

After #101 (read-only comparison) and #104 (HDR/shared upload component), the
next visual dependency is to see selected private images in the actual page.
D4b adds an on-demand mobile/desktop frame bound to the compared saved shop and
media/artwork metadata; changed inputs require deliberate reload/reselection.
Exact stamp artwork/credit stays in a separate collection-artwork view, preserving
the public header design. The approved-only D3 frame and every explicit writer
remain unchanged. This is a temporary preview, not a durable approval receipt.
Next: persist server-owned revision-bound review, then add explicit combined
publication with recoverable outcomes. Package D remains unfinished; catalogue
preparation is not started. No schema or hosted changes in this increment.
...
claim-token publication gates. It creates no catalogue source, verification date,
position attestation or private content field. It uses the established template
renderer and the same FNV palette/motif rules pinned to shop UUID rather than name.
A name-only draft still needs a stable unique slug; automatic slug assistance is
implemented in B2a for creation only; ordinary renames retain their URLs. Stamp readiness alone never makes an incomplete shop public.

| Checkpoint | Included / remaining |
| --- | --- |
...
| **B2a — Input and stamp compatibility checkpoint** | Shared full-document manual/mapped-import normalization, bounded field errors and focusable correction controls; creation-only stable URL assistance; optional uploaded creator credit through storage/API/issued snapshots; active stored generated artwork in public detail fixes PR #76’s post-merge review finding. B2a is merged/deployed with PR #77; founder usability acceptance is not complete. No import UI or trusted-publication claim. |
| **B2 — Shared editorial/publication contract** | Merged/deployed in PR #77 (founder usability acceptance not complete): shared manual/import normalization and structured field errors; trusted-editor publication without mandatory claim tokens, truthful public review attribution plus retained legacy sources, automatic actual reviewer/time, optional private notes/reference links, explicit position confirmation/invalidation, optional creator name/link validation across DB/API/collection decoders. Additive migrations/public decoding must ship with real editing and publication. |
| **B3 — Canonical vocabulary and essential editor** | Bounded locality/brand creation/reuse and country/locality consistency; reusable mappings once per distinct import value, seven directly reachable sections, essential field editing, precise blockers. Finish before C; may combine with B2 if reviewable. **B3a (frontend) is implemented** on `claude/nib-atlas-admin-rebuild-20rvho`: the seven approved sections, a persistent save/review action above the app navigation, feedback beside the work with dialogs reserved for decisions, a searchable IANA timezone field, explicit private/public image states with retry and honest removal wording, and position confirmation beside the coordinates. **B3b (backend) merged/deployed through #80/#79; founder reports checks 1–6 otherwise pass, except logo intake**: media removal, gallery cover/order/captions, capability advertisement and bounded locality/shop-type creation, all specified with acceptance cases in `docs/api/admin-b3-contract-handoff.md`. Brand/specialty creation remains available to editors; admins can create/reuse localities and shop types. New types carry compatible public labels. Independent technical review and founder acceptance are separate. |

This maps the full approved prototype to existing storage and the next additions;
the demo values/photos are never catalogue input. Preserve all current IDs,
source relationships, ordinary rename URLs, stamps and impressions.
...
B2 is deployed; founder mobile feedback requires the documented save/choice/upload/error correction before acceptance.

B3's seven-section essentials are implemented in the interface; canonical
locality preparation is implemented in the integration follow-up, pending acceptance before C. The shared normalizer consumes a complete document, not a
...
omitted/blank values, explicit clears, private-edit conflicts, grouped mapping,
bounded chunks and selected partial publication remain mandatory. B1's focused
200-default SQL check is **not** the 200-shop mixed-quality import acceptance or
Worker resource measurement. No CSV/JSON import interface ships in B1.
...
### Mobile usability correction after B2

Bounded Brand/Specialty creation/reuse is brought forward to remove empty
dropdown dead ends, with audited canonical identities. This does not complete
B3: locality creation, grouped vocabulary mapping and the essential seven-section
layout remain. C's mandatory 200-shop workflow, D's integrated media review and
all Package A/M5–M8 acceptance/deferral obligations remain unchanged.

### Package D4a — saved media/artwork comparison

The Review section can compare optional private photos, one logo and an uploaded
stamp draft alongside the current approved selections. It reuses the public gallery
and existing intact stamp-size previews. Choices exist only in component memory;
reload, leaving Review or a new saved shop revision resets them. Shop reads before
and after the separate media/stamp reads reject a changed saved revision. Media
and artwork are independently authorized current snapshots, not an atomic review
or a publication approval. Failed reads show no partial comparison.

This bounded increment makes no mutations, adds no API/schema, and preserves
D3's approved-media-only public-page frame and the approved experience artwork.
Existing separate photo/logo publishing and stamp activation remain deliberate.
