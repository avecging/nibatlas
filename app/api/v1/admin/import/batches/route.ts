...
import { handleImportBatch, ImportConflictError } from '@/src/server/admin/import-batch-http';
export const dynamic = 'force-dynamic';
async function route(request: Request) {
  try {
    const session = await createSupabaseServerClient(), gateway = await createAdminGateway(session);
    return await handleImportBatch(request, { ...gateway, async call(name, args) {
      ...
      if (error) throw Error('Import unavailable');
      return data;
    } });
