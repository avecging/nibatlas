...
export async function authorizeAdmin(
  ...
  minimum: AdminRole,
): Promise<AdminRole | Response> {
  const identity = await gateway.getIdentity();
  if (!identity || !isShopId(identity))
    return adminFailure("authentication_required");
...
function auditRows(value: unknown) {
  ...
  return value.map((item: unknown) => {
    ...
    const role = (value: unknown) =>
      value === "user" || value === "editor" || value === "admin";
    const catalogue =
      ["catalogue_insert", "catalogue_update", "catalogue_delete"].includes(
        String(row["action"]),
      ) &&
      [
        "shops",
    ...
    const summary = (value: Record<string, unknown> | null) => {
      ...
      for (const key of [
        ...
        if (v !== undefined) {
          if (
            typeof v !== "string" ||
            (key === "fingerprint" && !/^[a-f0-9]{32}$/.test(v)) ||
            (key === "publicationStatus" &&
              !["draft", "published", "archived"].includes(v)) ||
    ...
    if (
      ...
      !isShopId(row["entityId"]) ||
      typeof row["requestId"] !== "string" ||
      !isShopId(row["requestId"]) ||
      (row["actorUserId"] !== null &&
        (typeof row["actorUserId"] !== "string" ||
          !isShopId(row["actorUserId"]))) ||
      !["database_operator", "account"].includes(String(row["actorKind"])) ||
      (!catalogue &&
        (row["action"] !== "profile_role_changed" ||
    ...
    return {
      ...
      before: catalogue ? summary(before) : { role: before?.["role"] },
      after: catalogue ? summary(after) : { role: after?.["role"] },
